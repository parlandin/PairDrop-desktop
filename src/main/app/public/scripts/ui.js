class PeersUI {
  constructor() {
    ...
    Events.on("peer-connected", (e) =>
      this._onPeerConnected(e.detail.peerId, e.detail.connectionHash)
    );
    ...
    Events.on("activate-share-mode", (e) =>
      this._activateShareMode(e.detail.files, e.detail.text)
    );
    Events.on("translation-loaded", (_) => this._reloadShareMode());
    Events.on("room-type-removed", (e) =>
      this._onRoomTypeRemoved(e.detail.peerId, e.detail.roomType)
    );

    this.$shareModeCancelBtn.addEventListener("click", (_) =>
      this._deactivateShareMode()
    );

    Events.on("peer-display-name-changed", (e) =>
      this._onPeerDisplayNameChanged(e)
    );
  ...
  _changePeerDisplayName(peerId, displayName) {
    ...
    if (peerIdNode && displayName)
      peerIdNode.querySelector(".name").textContent = displayName;
  ...
  _redrawPeerRoomTypes(peerId) {
    ...
    Object.keys(peer._roomIds).forEach((roomType) =>
      peerNode.classList.add(`type-${roomType}`)
    );
  }
  ...
  async _activateShareMode(files = [], text = "") {
    if (this.shareMode.active || (files.length === 0 && text.length === 0))
      return;
    ...
    const sharedText = Localization.getTranslation(
      "instructions.activate-share-mode-shared-text"
    );
    const andOtherFilesPlural = Localization.getTranslation(
      "instructions.activate-share-mode-and-other-files-plural",
      null,
      { count: files.length - 1 }
    );
    const andOtherFiles = Localization.getTranslation(
      "instructions.activate-share-mode-and-other-file"
    );

    let descriptorComplete,
      descriptorItem,
      descriptorOther,
      descriptorInstructions;
    ...
    if (files.length > 0) {
      ...
      if (files.length > 1) {
        descriptorInstructions = Localization.getTranslation(
          "instructions.activate-share-mode-shared-files-plural",
          null,
          { count: files.length }
        );
      } else {
        descriptorInstructions = Localization.getTranslation(
          "instructions.activate-share-mode-shared-file"
        );
      }
      ...
      this.$shareModeEditBtn.addEventListener(
        "click",
        this._editShareTextCallback
      );
    ...
    const desktop = Localization.getTranslation(
      "instructions.x-instructions-share-mode_desktop",
      null,
      { descriptor: descriptorInstructions }
    );
    const mobile = Localization.getTranslation(
      "instructions.x-instructions-share-mode_mobile",
      null,
      { descriptor: descriptorInstructions }
    );
  ...
  async _deactivateShareMode() {
    ...
    const desktop = Localization.getTranslation(
      "instructions.x-instructions_desktop"
    );
    const mobile = Localization.getTranslation(
      "instructions.x-instructions_mobile"
    );
    ...
    this.$shareModeEditBtn.removeEventListener(
      "click",
      this._editShareTextCallback
    );
...
class PeerUI {
  ...
  constructor(peer, connectionHash, shareMode) {
    ...
    this._peer = peer;
    this._connectionHash = `${connectionHash.substring(
      0,
      4
    )} ${connectionHash.substring(4, 8)} ${connectionHash.substring(
      8,
      12
    )} ${connectionHash.substring(12, 16)}`;
    ...
    // ShareMode
    Events.on("share-mode-changed", (e) =>
      this._onShareModeChanged(e.detail.active, e.detail.descriptor)
    );
  }

  html() {
    let title = PeerUI._shareMode.active
      ? Localization.getTranslation("peer-ui.click-to-send-share-mode", null, {
          descriptor: PeerUI._shareMode.descriptor,
        })
  ...
  addTypesToClassList() {
    ...
    Object.keys(this._peer._roomIds).forEach((roomType) =>
      this.$el.classList.add(`type-${roomType}`)
    );
  ...
  _evaluateShareMode() {
    ...
    if (!PeerUI._shareMode.active) {
      ...
      title = Localization.getTranslation(
        "peer-ui.click-to-send-share-mode",
        null,
        { descriptor: PeerUI._shareMode.descriptor }
      );
  ...
  _bindListeners() {
    if (!PeerUI._shareMode.active) {
      ...
      this.$el
        .querySelector("input")
        .addEventListener("change", this._callbackInput);
...
class Dialog {
  ...
  _onPeerDisconnected(peerId) {
    if (this.isShown() && this.correspondingPeerId === peerId) {
      ...
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.selected-peer-left")
      );
    }
  }
...
class LanguageSelectDialog extends Dialog {
  ...
  selectLanguage(e) {
    ...
    Localization.setTranslation(languageCode).then((_) => this.hide());
  }
}
...
class ReceiveDialog extends Dialog {
  ...
  _parseFileData(
    ...
    if (files.length === 2) {
      ...
      fileOther = imagesOnly
        ? Localization.getTranslation(
            "dialogs.file-other-description-image-plural",
            null,
            { count: files.length - 1 }
          )
        : Localization.getTranslation(
            "dialogs.file-other-description-file-plural",
            null,
            { count: files.length - 1 }
          );
    }
    ...
    this.$fileStem.innerText = fileName.substring(
      0,
    ...
    this.$displayName.classList.remove(
      "badge-room-ip",
      "badge-room-secret",
      "badge-room-public-id"
    );
...
class ReceiveFileDialog extends ReceiveDialog {
  ...
  async _nextFiles() {
    ...
    const {
      ...
      totalSize,
      badgeClassName,
    } = this._filesQueue.shift();
    await this._displayFiles(
      peerId,
      displayName,
      connectionHash,
      files,
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (Object.keys(previewElement).indexOf(mime) === -1) {
          ...
          element.onerror = (_) => {
            reject(
              `${mime} preview could not be loaded from type ${file.type}`
            );
          };
  ...
  async _displayFiles(
    ...
    this.$receiveTitle.innerText = Localization.getTranslation(
      "dialogs.receive-title",
      null,
      { descriptor: descriptor }
    );

    const canShare =
      (window.iOS || window.android) &&
      !!navigator.share &&
      navigator.canShare({ files });
    if (canShare) {
      this.$shareBtn.removeAttribute("hidden");
      this.$shareBtn.onclick = (_) => {
        navigator.share({ files: files }).catch((err) => {
          console.error(err);
        });
      };
    }
    ...
    if (files.length > 1) {
      ...
      try {
        ...
        filenameDownload = `PairDrop_files_${year + month + date}_${
          hours + minutes
        }.zip`;
      } catch (e) {
        console.error(e);
    ...
    this.$downloadBtn.innerText =
      Localization.getTranslation("dialogs.download");
    this.$downloadBtn.onclick = (_) => {
      ...
      if (!canShare) {
        this.$downloadBtn.innerText = Localization.getTranslation(
          "dialogs.download-again"
        );
      }
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.download-successful", null, {
          descriptor: descriptor,
        })
      );
    ...
    document.title =
      files.length === 1
        ? `${Localization.getTranslation(
            "document-titles.file-received"
          )} - PairDrop`
        : `${Localization.getTranslation(
            "document-titles.file-received-plural",
            null,
            { count: files.length }
          )} - PairDrop`;
...
class ReceiveRequestDialog extends ReceiveDialog {
  constructor() {
    ...
    this.$acceptRequestBtn.addEventListener("click", (_) =>
      this._respondToFileTransferRequest(true)
    );
    this.$declineRequestBtn.addEventListener("click", (_) =>
      this._respondToFileTransferRequest(false)
    );

    Events.on("files-transfer-request", (e) =>
      this._onRequestFileTransfer(e.detail.request, e.detail.peerId)
    );
    Events.on("keydown", (e) => this._onKeyDown(e));
  ...
  _showRequestDialog(request, peerId) {
    ...
    if (
      request.thumbnailDataUrl &&
      request.thumbnailDataUrl.substring(0, 22) === "data:image/jpeg;base64"
    ) {
  ...
  _respondToFileTransferRequest(accepted) {
    ...
    if (accepted) {
      Events.fire("set-progress", {
        peerId: this.correspondingPeerId,
        progress: 0,
        status: "wait",
      });
      NoSleepUI.enable();
...
class InputKeyContainer {
  constructor(
    ...
    this.$inputKeyChars.forEach((char) =>
      char.addEventListener("input", (e) => this._onCharsInput(e))
    );
    this.$inputKeyChars.forEach((char) =>
      char.addEventListener("keydown", (e) => this._onCharsKeyDown(e))
    );
    this.$inputKeyChars.forEach((char) =>
      char.addEventListener("keyup", (e) => this._onCharsKeyUp(e))
    );
    this.$inputKeyChars.forEach((char) =>
      char.addEventListener("focus", (e) => e.target.select())
    );
    this.$inputKeyChars.forEach((char) =>
      char.addEventListener("click", (e) => e.target.select())
    );
  ...
  _onPaste(pastedKey) {
    ...
    pastedKey = pastedKey
      .replace(rgx, "")
      .substring(0, this.$inputKeyChars.length);
  ...
  _evaluateKeyChars() {
    if (
      this.$inputKeyContainer.querySelectorAll("input:placeholder-shown")
        .length > 0
    ) {
      ...
      const lastCharFocused =
        document.activeElement ===
        this.$inputKeyChars[this.$inputKeyChars.length - 1];
      if (lastCharFocused) {
...
class PairDeviceDialog extends Dialog {
  constructor() {
    ...
    this.$footerInstructionsPairedDevices = $$(
      ".discovery-wrapper .badge-room-secret"
    );
    ...
    this.$pairDeviceHeaderBtn.addEventListener("click", (_) =>
      this._pairDeviceInitiate()
    );
    ...
    Events.on("pair-device-initiated", (e) =>
      this._onPairDeviceInitiated(e.detail)
    );
    Events.on("pair-device-joined", (e) =>
      this._onPairDeviceJoined(e.detail.peerId, e.detail.roomSecret)
    );
    Events.on("peers", (e) => this._onPeers(e.detail));
    Events.on("peer-joined", (e) => this._onPeerJoined(e.detail));
    Events.on("pair-device-join-key-invalid", (_) =>
      this._onPublicRoomJoinKeyInvalid()
    );
    Events.on("pair-device-canceled", (e) =>
      this._onPairDeviceCanceled(e.detail)
    );
    Events.on("evaluate-number-room-secrets", (_) =>
      this._evaluateNumberRoomSecrets()
    );
    Events.on("secret-room-deleted", (e) =>
      this._onSecretRoomDeleted(e.detail)
    );
  ...
  _setKeyAndQRCode() {
    this.$key.innerText = `${this.pairKey.substring(
      0,
      3
    )} ${this.pairKey.substring(3, 6)}`;
  ...
  _copyPairUrl() {
    navigator.clipboard
      .writeText(this._getPairUrl())
      .then((_) => {
        Events.fire(
          "notify-user",
          Localization.getTranslation(
            "notifications.pair-url-copied-to-clipboard"
          )
        );
      })
      .catch((_) => {
        Events.fire(
          "notify-user",
          Localization.getTranslation("notifications.copied-to-clipboard-error")
        );
      });
  }
  ...
  _onPairDeviceJoined(peerId, roomSecret) {
    ...
    if (BrowserTabsConnector.peerIsSameBrowser(peerId)) {
      ...
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.pairing-tabs-error")
      );
      return;
    }
  ...
  _onPairPeerJoined(peerId, roomSecret) {
    ...
    PersistentStorage.addRoomSecret(roomSecret, displayName, deviceName)
      .then((_) => {
        Events.fire(
          "notify-user",
          Localization.getTranslation("notifications.pairing-success")
        );
        this._evaluateNumberRoomSecrets();
      })
      .finally(() => {
      ...
      .catch((_) => {
        Events.fire(
          "notify-user",
          Localization.getTranslation("notifications.pairing-not-persistent")
        );
  ...
  _onPublicRoomJoinKeyInvalid() {
    Events.fire(
      "notify-user",
      Localization.getTranslation("notifications.pairing-key-invalid")
    );
  }
  ...
  _onPairDeviceCanceled(pairKey) {
    Events.fire(
      "notify-user",
      Localization.getTranslation(
        "notifications.pairing-key-invalidated",
        null,
        { key: pairKey }
      )
    );
...
class EditPairedDevicesDialog extends Dialog {
  constructor() {
    ...
    this.$pairedDevicesWrapper = this.$el.querySelector(
      ".paired-devices-wrapper"
    );
    this.$footerBadgePairedDevices = $$(
      ".discovery-wrapper .badge-room-secret"
    );

    $("edit-paired-devices").addEventListener("click", (_) =>
      this._onEditPairedDevices()
    );
    this.$footerBadgePairedDevices.addEventListener("click", (_) =>
      this._onEditPairedDevices()
    );
  ...
  async _initDOM() {
    const pairedDeviceRemovedString = Localization.getTranslation(
      "dialogs.paired-device-removed"
    );
    const unpairString =
      Localization.getTranslation("dialogs.unpair").toUpperCase();
    const autoAcceptString = Localization.getTranslation(
      "dialogs.auto-accept"
    ).toLowerCase();
    const roomSecretsEntries =
      await PersistentStorage.getAllRoomSecretEntries();
    ...
    roomSecretsEntries.forEach((roomSecretsEntry) => {
                    ...
                    <div class="button-wrapper row fw center wrap">
                        <div class="center grow">
                            ...
                            <label class="auto-accept switch pointer m-1">
                                <input type="checkbox" ${
                                  roomSecretsEntry.auto_accept ? "checked" : ""
                                }>
      ...
      $pairedDevice
        ...
        .addEventListener("click", (e) => {
          PersistentStorage.updateRoomSecretAutoAccept(
            roomSecretsEntry.secret,
            e.target.checked
      ...
      $pairedDevice.querySelector("button").addEventListener("click", (e) => {
        PersistentStorage.deleteRoomSecret(roomSecretsEntry.secret).then(
          (roomSecret) => {
            Events.fire("room-secrets-deleted", [roomSecret]);
  ...
  _clearRoomSecrets() {
    PersistentStorage.getAllRoomSecrets().then((roomSecrets) => {
      PersistentStorage.clearRoomSecrets().finally(() => {
        ...
        Events.fire(
          "notify-user",
          Localization.getTranslation("notifications.pairing-cleared")
        );
        this.hide();
      });
    });
  ...
  _onPeerDisplayNameChanged(e) {
    ...
    PersistentStorage.updateRoomSecretNames(
      peer._roomIds["secret"],
      peer.name.displayName,
      peer.name.deviceName
    ).then((roomSecretEntry) => {
      console.log(
        `Successfully updated DisplayName and DeviceName for roomSecretEntry ${roomSecretEntry.key}`
      );
    });
  }
}
...
class PublicRoomDialog extends Dialog {
  constructor() {
    ...
    // disable temporarily functionality to create a public room
    ...
    Events.on("keydown", (e) => this._onKeyDown(e));
    Events.on("public-room-created", (e) =>
      this._onPublicRoomCreated(e.detail)
    );
    Events.on("peers", (e) => this._onPeers(e.detail));
    Events.on("peer-joined", (e) => this._onPeerJoined(e.detail));
    Events.on("public-room-id-invalid", (e) =>
      this._onPublicRoomIdInvalid(e.detail)
    );
  ...
  setFooterBadge() {
    ...
    this.$footerBadgePublicRoomDevices.innerText = Localization.getTranslation(
      "footer.public-room-devices",
      null,
      {
  ...
  _copyShareRoomUrl() {
    navigator.clipboard
      .writeText(this._getShareRoomUrl())
      .then((_) => {
        Events.fire(
          "notify-user",
          Localization.getTranslation(
            "notifications.room-url-copied-to-clipboard"
          )
        );
      })
      .catch((_) => {
  ...
  _onPublicRoomIdInvalid(roomId) {
    Events.fire(
      "notify-user",
      Localization.getTranslation("notifications.public-room-id-invalid")
    );
  ...
  _onPublicRoomLeft() {
    ...
    Events.fire(
      "notify-user",
      Localization.getTranslation("notifications.public-room-left", null, {
...
class SendTextDialog extends Dialog {
  constructor() {
    ...
    Events.on("text-recipient", (e) =>
      this._onRecipient(e.detail.peerId, e.detail.deviceName)
    );
    Events.on("keydown", (e) => this._onKeyDown(e));
  }
...
class ReceiveTextDialog extends Dialog {
  constructor() {
    ...
    Events.on("text-received", (e) =>
      this._onText(e.detail.text, e.detail.peerId)
    );
  ...
  _showReceiveTextDialog(text, peerId) {
    ...
    if (this.$text.innerText.length <= 300000) {
      ...
      const allowedDomainChars =
        "a-zA-Z0-9áàäčçđéèêŋńñóòôöšŧüžæøåëìíîïðùúýþćěłřśţźǎǐǒǔǥǧǩǯəʒâûœÿãõāēīōūăąĉċďĕėęĝğġģĥħĩĭįıĵķĸĺļľņňŏőŕŗŝşťũŭůűųŵŷżאבגדהוזחטיךכלםמןנסעףפץצקרשתװױײ";
      const urlRgx = new RegExp(
        `(^|\\n|\\s|["><\\-_~:\\/?#\\[\\]@!$&'()*+,;=%.])(((https?:\\/\\/)?(?:[${allowedDomainChars}](?:[${allowedDomainChars}-]{0,61}[${allowedDomainChars}])?\\.)+[${allowedDomainChars}][${allowedDomainChars}-]{0,61}[${allowedDomainChars}])(:?\\d*)\\/?([${allowedDomainChars}_\\/\\-#.]*)(\\?([${allowedDomainChars}\\-_~:\\/?#\\[\\]@!$&'()*+,;=%.]*))?)`,
        "g"
      );

      $textShadow.innerText = text.replace(
        urlRgx,
      ...
      this.$text.innerHTML = $textShadow.innerHTML.replace(pRgx, (m) => {
  ...
  _setDocumentTitleMessages() {
    document.title =
      this._receiveTextQueue.length <= 1
        ? `${Localization.getTranslation(
            "document-titles.message-received"
          )} - PairDrop`
        : `${Localization.getTranslation(
            "document-titles.message-received-plural",
            null,
            { count: this._receiveTextQueue.length + 1 }
          )} - PairDrop`;
  }
  ...
  async _onCopy() {
    ...
    navigator.clipboard
      ...
      .then((_) => {
        Events.fire(
          "notify-user",
          Localization.getTranslation("notifications.copied-to-clipboard")
        );
        this.hide();
      })
      .catch((_) => {
...
class ShareTextDialog extends Dialog {
  constructor() {
    ...
    this.$approveMsgBtn.addEventListener("click", (_) =>
...
class Base64Dialog extends Dialog {
  ...
  async evaluateBase64Text(base64Text, hash) {
    this.$title.innerText = Localization.getTranslation(
      "dialogs.base64-title-text"
    );
  ...
  _setPasteBtnToProcessing() {
    ...
    this.$pasteBtn.innerText = Localization.getTranslation(
      "dialogs.base64-processing"
    );
  }
  ...
  preparePasting(type) {
    ...
    if (navigator.clipboard.readText) {
      ...
      console.log(
        "`navigator.clipboard.readText()` is not available on your browser.\nOn Firefox you can set `dom.events.asyncClipboard.readText` to true under `about:config` for convenience."
      );
      this.$pasteBtn.setAttribute("hidden", true);
      this.$fallbackTextarea.setAttribute(
        "placeholder",
        Localization.getTranslation("dialogs.base64-paste-to-send", null, {
          type: translateType,
        })
      );
      this.$fallbackTextarea.removeAttribute("hidden");
      this._inputCallback = (_) => this.processInput(type);
      this.$fallbackTextarea.addEventListener("input", (_) =>
        this._inputCallback()
      );
  ...
  async processPastedBase64(type, base64) {
    try {
      ...
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.clipboard-content-incorrect")
      );
  ...
  async processBase64Text(base64) {
    ...
    try {
      ...
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.text-content-incorrect")
      );
  ...
  async processBase64Zip(base64) {
    ...
    try {
      ...
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.file-content-incorrect")
      );
  ...
  hide() {
    ...
    this.$fallbackTextarea.removeEventListener("input", (_) =>
      this._inputCallback()
    );
...
class AboutUI {
  ...
  async _onConfig(btnConfig) {
    ...
    await this._evaluateBtnConfig(
      this.$privacypolicyBtn,
      btnConfig.privacypolicy_button
    );
  }
...
class Notifications {
  constructor() {
    ...
    this.$headerNotificationButton.addEventListener("click", (_) =>
    ...
    await Notification.requestPermission((permission) => {
      if (permission !== "granted") {
        Events.fire(
          "notify-user",
          Localization.getTranslation(
            "notifications.notifications-permissions-error"
          )
        );
        return;
      }
      Events.fire(
        "notify-user",
        Localization.getTranslation("notifications.notifications-enabled")
      );
  ...
  _messageNotification(message, peerId) {
    if (document.visibilityState !== "visible") {
      ...
      if (
        /^((https?:\/\/|www)[abcdefghijklmnopqrstuvwxyz0123456789\-._~:\/?#\[\]@!$&'()*+,;=]+)$/.test(
          message.toLowerCase()
        )
      ) {
        const notification = this._notify(
          Localization.getTranslation("notifications.link-received", null, {
            name: peerDisplayName,
          }),
          message
        );
        this._bind(notification, (_) =>
          window.open(message, "_blank", "noreferrer")
        );
      } else {
        const notification = this._notify(
          Localization.getTranslation("notifications.message-received", null, {
            name: peerDisplayName,
          }),
          message
        );
    ...
    if (document.visibilityState !== "visible") {
      ...
      if (files.length === 1) {
        ...
        if (files.length === 2) {
          fileOther = imagesOnly
            ? Localization.getTranslation(
                "dialogs.file-other-description-image"
              )
            : Localization.getTranslation(
                "dialogs.file-other-description-file"
              );
        } else {
          fileOther = imagesOnly
            ? Localization.getTranslation(
      ...
      const notification = this._notify(
        title,
        Localization.getTranslation("notifications.click-to-download")
      );
  ...
  _requestNotification(request, peerId) {
    if (document.visibilityState !== "visible") {
      let imagesOnly = request.header.every(
        (header) => header.mime.split("/")[0] === "image"
      );
      ...
      let title = Localization.getTranslation(
        "notifications.request-title",
        null,
        {
          name: displayName,
      ...
      const notification = this._notify(
        title,
        Localization.getTranslation("notifications.click-to-show")
      );
    }
  }
  ...
  async _copyText(message, notification) {
    if (await navigator.clipboard.writeText(message)) {
      ...
      this._notify(
        Localization.getTranslation("notifications.copied-text-error")
      );
    }
  }
  ...
  _bind(notification, handler) {
    if (notification.then) {
      notification.then((_) => {
        serviceWorker.getNotifications().then((_) => {
...
class NetworkStatusUI {
  ...
  _showOnlineMessage() {
    Events.fire(
      "notify-user",
      Localization.getTranslation("notifications.online")
    );
  }
}
      ...
      openRequest.onsuccess = (e) => {
        ...
        request.onsuccess = (_) => {
          ...
          for (let i = 0; i < fileObjects.length; i++) {
            filesReceived.push(
              new File([fileObjects[i].buffer], fileObjects[i].name)
            );
          }
  ...
  async evaluateLaunchQueue() {
    ...
    launchQueue.setConsumer(async (launchParams) => {
      ...
      for (let i = 0; i < launchParams.files.length; i++) {
        if (
          i !== 0 &&
          (await launchParams.files[i].isSameEntry(launchParams.files[i - 1]))
        )
          continue;
