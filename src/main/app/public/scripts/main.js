class PairDrop {
  constructor() {
    ...
    this.$footerPairedDevicesBadge = $$(
      ".discovery-wrapper .badge-room-secret"
    );
    this.$headerInstallBtn = $("install");

    this.deferredStyles = ["styles/styles-deferred.css"];
  ...
  async initialize() {
    ...
    console.log(
      "Evaluate URL params as soon as websocket connection is established."
    );
  ...
  loadDeferredAssets() {
    const stylePromises = this.deferredStyles.map((url) =>
      this.loadAndApplyStylesheet(url)
    );
    const scriptPromises = this.deferredScripts.map((url) =>
      this.loadAndApplyScript(url)
    );
  ...
  async evaluateUrlParams() {
    ...
    if (urlParams.has("pair_key")) {
      ...
      await this.webShareTargetUI.evaluateShareTarget(
        shareTargetType,
        title,
        text,
        url
