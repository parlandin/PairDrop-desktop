...
const forceFetch = true; // FOR DEVELOPMENT: Set to true to always update assets instead of using cached versions
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(cacheTitle).then(function (cache) {
      return cache.addAll(relativePathsToCache).then((_) => {
  ...
  new Promise((resolve, reject) => {
    ...
    fetch(request)
      .then((response) => {
        ...
        update(request)
          .then(() =>
            console.log("Cache successfully updated for", request.url)
          )
          .catch((reason) =>
            console.log(
              "Cache could not be updated for",
              request.url,
              "Reason:",
              reason
            )
...
const fromCache = (request) =>
  caches.open(cacheTitle).then((cache) => cache.match(request));

const rootUrl = location.href.substring(
  0,
  location.href.length - "service-worker.js".length
);
...
const update = (request) =>
  new Promise((resolve, reject) => {
    if (doNotCacheRequest(request)) {
      reject(
        "Url is specifically prevented from being cached in the serviceworker."
      );
      return;
    }
        ...
        for (let i = 0; i < fileObjects.length; i++) {
          ...
          objectStoreRequest.onsuccess = (_) => {
            if (i === fileObjects.length - 1)
              resolve(pairDropUrl + "?share_target=files");
          };
        }
      };
