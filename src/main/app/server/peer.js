...
export default class Peer {
  ...
  _setIP(request) {
    ...
    if (this.ip.substring(0, 7) === "::ffff:") this.ip = this.ip.substring(7);
    ...
    if (this.conf.debugMode) {
      ...
      if (ipv6_was_localized) {
        console.debug(
          "IPv6 client IP was localized to",
          this.conf.ipv6Localize,
          this.conf.ipv6Localize > 1 ? "segments" : "segment"
        );
      }
  ...
  _setPeerId(request) {
    ...
    if (
      peerId &&
      Peer.isValidUuid(peerId) &&
      this.isPeerIdHashValid(peerId, peerIdHash)
    ) {
      this.id = peerId;
    } else {
  ...
  _setName(req) {
    ...
    if (ua.ua == "pairDrop-desk") {
      deviceName = "PairDrop Desktop";
    }
  ...
  static isValidUuid(uuid) {
    return /^([0-9]|[a-f]){8}-(([0-9]|[a-f]){4}-){3}([0-9]|[a-f]){12}$/.test(
      uuid
    );
  }
