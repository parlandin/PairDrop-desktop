...
conf.wsFallback =
  ...
  process.env.WS_FALLBACK === "true";

conf.rtcConfig =
  process.env.RTC_CONFIG && process.env.RTC_CONFIG !== "false"
...
conf.signalingServer =
  process.env.SIGNALING_SERVER && process.env.SIGNALING_SERVER !== "false"
...
if (conf.signalingServer) {
  const isValidUrl = /[a-z|0-9|\-._~:\/?#\[\]@!$&'()*+,;=]+$/.test(
    conf.signalingServer
  );
  ...
  if (process.env.RTC_CONFIG || conf.wsFallback || conf.ipv6Localize) {
    console.error(
      "SIGNALING_SERVER cannot be used alongside WS_FALLBACK, RTC_CONFIG or IPV6_LOCALIZE as these " +
...
// Print all IPs with port
ips.forEach((ip) => {
  console.log("\nPairDrop is running on: ", `http://${ip}:${conf.port}`);
});
